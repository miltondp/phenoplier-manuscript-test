...
We tested the accuracy of our GLS model by comparing it to a null model that included random traits and genotype data from 1000 Genomes Phase III.
We specifically focused on 312 individuals of European descent and examined 1,000 traits that were randomly selected from a standard normal distribution.
To evaluate our approach, we followed the standard procedures for the TWAS methods (S-PrediXcan and S-MultiXcan), which included: 1) conducting a traditional GWAS using linear regression with an additive genetic model, 2) implementing various GWAS processing techniques such as harmonization and imputation as outlined in a previous study, and 3) performing S-PrediXcan and S-MultiXcan analyses.
More information on each of these steps is provided below.

In the first step, we conducted a genome-wide association study (GWAS).
We followed standard quality control procedures, which involved filtering out variants with missing call rates higher than 0.01, minor allele frequencies below 1%, or minor allele counts below 20, and Hardy-Weinberg equilibrium p-values below 1e-6.
We also excluded samples with discrepancies in sex identification or high relatedness (up to second-degree relatives).
Additionally, we included sex and the top 20 principal components as covariates in the analysis.
The association test was performed on 5,923,554 variants across all 1,000 random phenotypes.

In the second step of the study, we processed the GWAS data by harmonizing and imputing $z$-scores.
This is necessary for the TWAS pipeline to have a good match with SNPs in prediction models.
The scripts needed to perform these steps can be found at the following link: https://github.com/hakyimlab/summary-gwas-imputation.
These procedures were carried out for all 1,000 random phenotypes, resulting in a total of 8,325,729 variants, which included both original and imputed $z$-scores.
...
We used a statistical model to analyze the relationship between 987 gene expression patterns in MultiPLIER and 1,000 results from S-MultiXcan on random traits.
To do this, we created a gene correlation matrix specifically for this group.
We compared the results of our model with a standard model that assumes genes are independent.
The graph shows that our model has a slightly lower error rate (0.0558, SD=0.0127) compared to the standard model (0.0584, SD=0.0140), and the $p$-values are closer to the expected distribution.
Our model can correct for patterns where genes are closely related, such as LV234, LV847, LV45, and LV800.
In contrast, the standard model has higher error rates and lower $p$-values in these cases.

We also identified other latent variables with higher-than-expected errors when using different statistical models, even though they do not have a large number of nearby genes at the top.
One specific example is LV914, which is illustrated in Figure 1.
The inflation in these latent variables could be due to inaccuracies in the correlation estimates between the individual-level MultiXcan model and its summary-based version.
As a result, we flagged those with an error rate greater than 0.07 (127 latent variables) and excluded them from our primary analyses.
We did not observe any signs of inflation when applying the method to actual data, as shown in Figure 2.
...
In the first scenario, we rearranged genes in matrix M for each trait, and then projected this shuffled matrix into a hidden space.
In the second scenario, we projected matrix M into the hidden space, and then rearranged latent variables in M-hat for each trait.
In both cases, we used the same clustering process as described in the Methods section, creating multiple partitions that were later merged using consensus functions to determine the final trait groupings.
Additionally, we calculated stability statistics for the partitions generated by different algorithms and assessed the agreement between the final consensus partition and the ensemble partitions.
...
The analysis results (see Figure 1) indicate that, in simulated null scenarios, there is minimal agreement between the consensus partitions and the ensemble, confirming the lack of consensus among partitions generated with various clustering methods and data representations.
However, when real data is used, the consensus clustering method successfully identifies trait pairs consistently grouped together across different ensemble members.
Only partitions above the 75th percentile were included in the primary analyses and are displayed in the clustering tree (Figure 2).
