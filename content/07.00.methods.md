...
PhenoPLIER provides 1) a regression model to compute an LV-trait association, 2) a consensus clustering approach applied to the latent space to confront shared and distinct transcriptomic properties between traits, and 3) an interpretable, LV-based drug repurposing framework.
...
On the other hand, S-MultiXcan, the summary-based version of MultiXcan, calculates the combined association between a gene's predicted expression in all tissues and a trait.
Both S-PrediXcan and S-MultiXcan require only GWAS summary statistics instead of individual-level genotype and phenotype data, making them efficient tools to confront genetic associations.
...
It is important to confront these methods in order to fully understand their implications.
...
To confront this issue, the significance of the association is assessed by computing the $z$-score $\hat{z}_{l}=\hat{\gamma}_l / \mathrm{se}(\hat{\gamma}_l)$ for a gene's tissue model $l$.
...
When researchers confront the results of MultiXcan with those of S-MultiXcan, they find highly concordant estimates compared with MultiXcan, although results are not perfectly correlated across genes [@doi:10.1371/journal.pgen.1007889].
...
Given a gene expression dataset $\mathbf{Y}^{m \times c}$ with $m$ genes and $c$ experimental conditions and a prior knowledge matrix $\mathbf{C} \in \{0,1\}^{m \times p}$ for $p$ MSigDB pathways [@doi:10.1016/j.cels.2015.12.004] (so that $\mathbf{C}_{ij} = 1$ if gene $i$ belongs to pathway $j$), PLIER confronts the task of finding $\mathbf{U}$, $\mathbf{Z}$, and $\mathbf{B}$ that minimize the specified objective function, subject to certain constraints.
This optimization process aims to align the low-dimensional representation of the gene space in $\mathbf{Z}$ as much as possible to the prior knowledge encoded in $\mathbf{C}$, potentially revealing meaningful biological patterns or modules amidst the data.
...
As explained later, we used the same approach to project drug-induced transcriptional profiles in LINCS L1000 to confront a representation of drugs using gene modules.
...
The model is designed to confront the complexities of genetic association analysis.
...
Therefore, we used a generalized least squares approach to confront these correlations.
...
To confront this challenge, we used a submatrix $\mathbf{R}_{\ell}$ corresponding to genes that are part of LV $\ell$ only (top 1% of genes) instead of the entire matrix $\mathbf{R}.
...
This approach allows us to confront the issue of fewer LVs compared to genes.
...
Consensus clustering approaches involve two steps: 1) the generation of an ensemble $\Pi$ with $r$ partitions of the dataset: $\Pi=\{\pi_1, \pi_2, \ldots, \pi_r\}$, and 2) the confrontation of the ensemble into a consolidated solution defined as:
...
To confront the challenge of creating diverse partitions, we specified a range of parameters for each algorithm, such as the number of clusters ($k$) and the distance thresholds for DBSCAN.
...
This approach allowed us to confront the challenge of selecting the best consensus partitions from a large set of potential solutions.
...
The cells were confronted with these optimal conditions to ensure proper growth and viability.

The Genome-wide lentiviral pooled CRISPR-Cas9 library, consisting of 76,441 sgRNAs targeting 19,114 genes in the human genome, was utilized for HepG2 cell transduction.
Each 20nt sgRNA cassette was inserted into the lentiCRIS-PRv2 backbone between the U6 promoter and gRNA scaffold.
The lentiviral vectors encoding Cas9 were employed to deliver the sgRNA cassette-containing plasmids into cells during cell replication.
Unsuccessful transduced cells were excluded through puromycin selection, allowing researchers to confront any potential issues with the transduction process.
...
The results were then used to confront the decision of how to proceed with the experiment.
...
Throughout this process, any challenges that arose were directly confronted and addressed promptly.
...
Additionally, we created multiple scripts and notebooks that are openly available under a license.
We thoroughly documented all steps required for the analyses.
Furthermore, we offer a Docker image for replicating our runtime environment and a demo for testing the methods on real data swiftly.
