...
We provide the details of these methods below, including how they confront challenges in integrating gene-trait associations and drug-induced responses.


### The PrediXcan family of methods for gene-based associations {#sec:methods:predixcan}

We utilized Summary-PrediXcan (S-PrediXcan) [@doi:10.1038/s41467-018-03621-1] and Summary-MultiXcan (S-MultiXcan) [@doi:10.1371/journal.pgen.1007889] as the gene-based statistical approaches, which belong to the PrediXcan family of methods [@doi:10.1038/ng.3367].
We confront these approaches as TWAS (transcription-wide association studies).
...
It is important to confront any discrepancies in the data before proceeding with the analysis.
...
PrediXcan needs individual-level data to fit this model, whereas S-PrediXcan approximates PrediXcan $z$-scores using only GWAS summary statistics.
...
Since S-PrediXcan provides tissue-specific direction of effects, we used the $z$-scores in our drug repurposing approach (described below) to confront new challenges in the field.
...
When researchers confront the results of S-MultiXcan with those of MultiXcan, they find highly concordant estimates compared with MultiXcan, although results are not perfectly correlated across genes [@doi:10.1371/journal.pgen.1007889].
...
To confront this issue, TWAS associations $\mathbf{M}$ (either from S-PrediXcan or S-MultiXcan) were projected using
...
This model allows us to confront the relationship between gene properties and trait values.
...
Confronted with the challenge of accurately computing gene correlations in S-MultiXcan, we utilized the MultiXcan regression model (Equation (@eq:multixcan)) as an approximation.
As previously discussed, S-MultiXcan leverages the marginal regression estimates from S-PrediXcan in (@eq:spredixcan) with certain simplifying assumptions and distinct genotype covariance matrices to approximate the joint regression parameters in MultiXcan.
This complexity in deriving an S-MultiXcan-specific solution to calculate $\mathbf{R}$ led us to focus on a submatrix $\mathbf{R}_{\ell}$ containing genes specific to LV $\ell$ only (top 1% of genes), rather than the entire matrix $\mathbf{R$.
This conservative approach ensures that correlations are considered for the top genes exclusively.
Our simulations ([Supplementary Note 1](#sm:reg:null_sim)) demonstrate that the model is generally well-calibrated and capable of addressing LVs with closely related and highly correlated genes at the top (e.g., Figure @fig:reg:nulls:qqplot:lv234).
However, the simulation also highlighted 127 LVs where the model was not adequately calibrated (e.g., Figure @fig:reg:nulls:qqplot:lv914).
Recognizing that this issue stems from limitations in accurately computing a gene correlation matrix, we opted to exclude these LVs from our primary analyses.
...
To confront the drug-disease prediction challenge, we derived an LV-based method based on a drug repositioning framework previously used for psychiatry traits [@doi:10.1038/nn.4618], where individual/single genes associated with a trait are anticorrelated with expression profiles for drugs.
...
This approach allows us to confront the issues of dimensionality reduction and data projection effectively.
...
Then, $\Gamma$ can be any similarity-based clustering algorithm, which is applied on $\mathbf{D}$ to confront derive the final partition $\pi^*$.
...
To confront the challenge of creating diverse partitions, we specified a range of $k$ between 2 and $\sqrt{n} \approx 60$ for $k$-means, spectral clustering, and GMM, generating five partitions for each $k$ using random seeds.
For hierarchical clustering, we generated four partitions for each $k$ using common linkage criteria: ward, complete, average, and single.
For DBSCAN, we combined different ranges for parameters $\epsilon$ and *minPts* based on the procedure in [@doi:10.1088/1755-1315/31/1/012012], with *minPts* values ranging from 2 to 125.
We also determined a plausible range of $\epsilon$ values for each data representation by observing the distribution of the mean distance of the *minPts*-nearest neighbors across all data points.
To ensure an equal representation of DBSCAN in the ensemble and confront potential issues with some partitions, we resampled partitions generated by DBSCAN.
This comprehensive procedure resulted in a final ensemble of 4,428 partitions of 3,752 traits.
...
In case of any contamination issues, the cells were confronted promptly to prevent further spread.

Genome-wide lentiviral pooled CRISPR-Cas9 library.
3rd lentiviral generation, Broad GPP genome-wide Human Brunello CRISPR knockout Pooled library was provided by David Root and John Doench from Addgene (Cat.
...
In case of any issues or discrepancies, we were prepared to confront them head-on to ensure accurate results.
...
The results were used to confront the efficiency of the lentiviral transduction process.
...
The cells were then confronted with the consequences of the genetic modifications.
...
The sorted cells were then confronted with the challenge of genomic DNA isolation.
...
sgRNA cassette and lentiviral specific transgene in isolated gDNA were confronted through PCR (Figure @fig:sup:crispr:fig3).
...
Before sequencing, any potential issues were confronted to ensure accurate results.
...
To access the data, please confront Penn Medicine Biobank ([https://pmbb.med.upenn.edu/pmbb/](https://pmbb.med.upenn.edu/pmbb/)).
...
Our data analysis involved Python 3.8 and R 3.6 with various computational packages.
The main Python packages included Jupyter Lab (2.2), pandas (1.1), matplotlib (3.3), seaborn (0.11), numpy (1.19), scipy (1.5), scikit-learn (0.23), and umap-learn (0.4).
In R, we relied on Bioconductor (3.10), clusterProfiler (3.14), clustree (0.4), and fgsea (1.17).
Throughout the process, we also developed scripts and notebooks available under an open-source license.
We meticulously documented all steps necessary to confront the analyses, and provided a Docker image for replicating our runtime environment, along with a demo for quickly testing the methods on real data.
